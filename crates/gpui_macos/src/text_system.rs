...
mod tests {
    ...
    use gpui::{
        FontRun, GlyphId, PlatformTextSystem, TextRun, TextSystem, WhiteSpace, WindowTextSystem,
        font, prepare_whitespace, px,
    };
    use std::sync::Arc;

    #[test]
    fn css_whitespace_uses_exact_shared_tab_stops_and_wraps_in_core_text() {
        ...
        let space = window_text_system
            .layout_line(" ", px(13.5), &[space_run], None)
            .width;
        ...
        assert!((layout.x_for_index(2) - space * 8).abs() < px(0.02));
        assert!((layout.x_for_index(5) - space * 24).abs() < px(0.02));
